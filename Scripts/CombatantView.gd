# CombatantView.gd - Updated for effective stats
...
func setup(new_combatant: Combatant, is_player_combatant: bool = true):
    ...
    if combatant:
        ...
        # Initial update of status effects
...
func update_hp_display():
    if combatant:
        var max_hp = combatant.get_effective_max_hp()
        var hp_ratio = float(combatant.current_hp) / float(max_hp)
...
func update_mp_display():
    if combatant and combatant.max_mp > 0:
        var max_mp = combatant.get_effective_max_mp()
        var mp_ratio = float(combatant.current_mp) / float(max_mp)
...
func update_status_effects():
    ...
    print("Updating status effects for: " + combatant.display_name)
    print("Status effects count: " + str(combatant.status_effects.size()))
    ...
    for effect in combatant.status_effects:
        ...
        # Build detailed tooltip showing stat modifications
        var tooltip = effect.name + " (" + str(effect.remaining_turns) + " turns)\n" + effect.description
        
        # Add stat modification info to tooltip
        if effect.modify_physical_attack != 0:
            tooltip += "\nPhysical Attack: " + ("+" if effect.modify_physical_attack > 0 else "") + str(effect.modify_physical_attack) + "%"
        if effect.modify_magic_attack != 0:
            tooltip += "\nMagic Attack: " + ("+" if effect.modify_magic_attack > 0 else "") + str(effect.modify_magic_attack) + "%"
        if effect.modify_physical_defense != 0:
            tooltip += "\nPhysical Defense: " + ("+" if effect.modify_physical_defense > 0 else "") + str(effect.modify_physical_defense) + "%"
        if effect.modify_magic_defense != 0:
            tooltip += "\nMagic Defense: " + ("+" if effect.modify_magic_defense > 0 else "") + str(effect.modify_magic_defense) + "%"
        if effect.modify_speed != 0:
            tooltip += "\nSpeed: " + ("+" if effect.modify_speed > 0 else "") + str(effect.modify_speed) + "%"
...
func _on_status_effect_added(effect):
    print("Status effect added to " + combatant.display_name + ": " + effect.name)
    update_status_effects()

func _on_status_effect_removed(effect):
    print("Status effect removed from " + combatant.display_name + ": " + effect.name)
    update_status_effects()
