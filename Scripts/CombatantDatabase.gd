...
func create_default_combatants() -> void:
	...
	# Example of mixing code-created and resource-loaded abilities
	warrior.abilities = [
		AbilityFactory.create_damage_ability("Slash", 15, Ability.TargetType.ENEMY, "A strong sword slash"),
		load_ability_from_resource("res://Resources/Abilities/heavy_strike.tres"), # Example resource path
	...
	# You can also load abilities from resource folders
	wizard.abilities = []
	
	# Example of loading all abilities from a directory
	var dir = DirAccess.open("res://Resources/Abilities/Wizard")
	if dir:
		dir.list_dir_begin()
		var file_name = dir.get_next()
		while file_name != "":
			if file_name.ends_with(".tres"):
				var ability = load_ability_from_resource("res://Resources/Abilities/Wizard/" + file_name)
				if ability:
					wizard.abilities.append(ability)
			file_name = dir.get_next()
	else:
		# Fallback to code-defined abilities if directory access fails
		wizard.abilities = [
